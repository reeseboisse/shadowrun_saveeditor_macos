...
12. **Attributes in `CharacterMod.stats` are a *modifier over a content-pack base*, not the effective value.** (Investigated and **resolved** in Phase 5 via content-pack extraction.) The player's `CharacterInstance.character_sheet_id` is `"CG <Race> None"` (e.g. `CG Elf None`), a character-generation racial base sheet. The DLL exposes `SetInstanceModFromBaseAttributes` / `baseAttributes`: the engine computes effective attributes as **base-sheet value + stored modifier**, and the save stores only the modifier. Evidence it's a modifier and not an absolute value: attributes a character never raised are stored as `0` and therefore omitted by protobuf-net (gotcha 7) — Cooma's strength/willpower, Coomer's intelligence — and no character can have an absolute attribute of 0. Confirmed against the extracted `CG Elf None` sheet (charisma 2, essence 6, others 1, reaction 4): HK Coomer's stored mods + that base give sane effective stats (body 6, quickness 7, charisma 7, etc.). **Resolution:** the games keep content as loose files (no `.cpack` archive); `content_extractor.py` reads `chars/cg <race> none.ch_sht.bytes` (Character message, tag 4 = stats) and records the racial base, and the service resolves effective = base + modifier on read and writes (value − base) on edit. This is gated on the catalog being present (generated locally, gitignored — item descriptions are game text); with no catalog the editor shows the raw modifier as before. Derived attributes (`reaction`, `essence`) remain excluded from the editable surface; see `DERIVED_ATTRIBUTES`.
